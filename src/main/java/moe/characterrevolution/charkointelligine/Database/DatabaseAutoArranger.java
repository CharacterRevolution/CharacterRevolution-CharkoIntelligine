...
class ImageCleaner implements Runnable {
     ...
     */
    ImageCleaner(CharkoIntelligine charkointelligine, List<File> fileList) {
     ...
     */
    public void run() {
        File imageFolder = new File("data/CharkoIntelligine/images/");
        if(imageFolder.exists()) {
            ...
            charkointelligine.getLogger().info("数据库整理器已清理未使用的图片缓存：总计" + imageFiles.size() + "个图片");
        }
    }
...
public class DatabaseAutoArranger extends TimerTask {
    ...
    public List<File> Text2ImageFile(String text) {
        ...
        while(mt.find()) {
            ...
            File file = new File("data/CharkoIntelligine/images/", imageId);
    ...
    public void run() {
        ...
        ImageCleaner imageCleaner = new ImageCleaner(charkointelligine, fileList);
        imageCleaner.start();
